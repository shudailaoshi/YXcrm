package com.YXcrm.system.controller;
...
public class RoleResourceController extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (qqiu.equals("test") || qqiu.equals("add") || qqiu.equals("delete")
			...
			if (str != null && str != "" && str.length() != 0) { // 非空判断，防止前台传空报500服务器错误中的空指针
				...
				roleResource = t_map2bean.MapToRoleResource(map);
			} else {
				System.out.println("前台传入post请求体数据为空，请联系管理员！");
			}

			// 3 执行qqiu里面的增或删或改或查 的操作
			qqiuChoice(qqiu, roleResource);
		} else if (qqiu.equals("list")) {
			// TODO 待完成
			ArrayList<RoleResource> resultList = roleResourceService.getList();
			backResult.setMessage("信息值：成功");
			backResult.setQingqiu("list查询列表");
			backResult.setData(resultList);

		} else if (qqiu.equals("rs")) {// 接收用户-角色中传过来的角色id集合
			// List roleList=(List)request.getAttribute("list");
			List roleList = new ArrayList();
			roleList.add("u001");
			roleList.add("u002");
			List<String> resultList = roleResourceService
					.getRsbyRoleid(roleList);
			backResult.setMessage("信息值：成功！");
			backResult.setQingqiu("所有资源id查询");
			backResult.setData((ArrayList<?>) resultList);
			...
			if (str != null && str != "" && str.length() != 0) {
				...
				Gson gson = new Gson();
				ArrayList<RoleResource> pr_List = new ArrayList<RoleResource>();

				// 加强for循环遍历JsonArray
				for (JsonElement one : jsonArray) {
					// 使用GSON，直接转成Bean对象
					RoleResource pr = gson.fromJson(one, RoleResource.class);
					pr_List.add(pr);
				}
				System.out.println("数组转换出来的列表数据!!!!!" + pr_List);
				// end前台数据转换
				String count = roleResourceService.insert_batch(pr_List);
				...
				resultList.add("前台传入的数据为空,或者格式不对,请联系前端开发!");
				backResult.setData(resultList);
			}
	...
	private void qqiuChoice(String qqiu, RoleResource roleResource) {
		// TODO Auto-generated method stub
		boolean test = false;
		boolean add = false;
		...
		if (add) {
			String result = roleResourceService.insert(roleResource);
			ArrayList<String> resultList = new ArrayList<String>();
		...
		if (delete) {
			String result = roleResourceService.delete(roleResource.getUuid());
			ArrayList<String> resultList = new ArrayList<String>();
			resultList.add(result);
			backResult.setMessage("信息值：成功");
			backResult.setQingqiu("delete删除" + roleResource.getUuid());
			backResult.setData(resultList);
		}
		if (edit) {
			String result = roleResourceService.update(roleResource);
			ArrayList<String> resultList = new ArrayList<String>();
			resultList.add(result);
			backResult.setMessage("信息值：成功");
		...
		if (getOne) {
			RoleResource result = roleResourceService.getByUuid(roleResource
					.getUuid());
			ArrayList<RoleResource> resultList = new ArrayList<RoleResource>();
			resultList.add(result);
			backResult.setMessage("信息值：成功");
